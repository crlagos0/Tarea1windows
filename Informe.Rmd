...
Con respecto a la posible influencia del nivel educacional en la prevalencia de eventos coronarios, se vio una diferencia significativa entre los grupos (`r chisq.test(base$education, base$TenYearCHD)`). Esto se puede apreciar en el gráfico 2. 


```{r echo=FALSE}
library(tidyverse)
edCHD <- base %>% group_by(education, TenYearCHD) %>%  summarise(nCHD = n()) %>% mutate(Porcentaje = nCHD*100/sum(nCHD))
ggplot(edCHD, aes(x=education, y=Porcentaje)) + geom_col(aes(fill = TenYearCHD)) + theme_classic()
```
